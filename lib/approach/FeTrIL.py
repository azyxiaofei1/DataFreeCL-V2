...
class FeTrIL_handler:
    ...
    def handler_init(self):
        ...
        self.trainer = FeTrIL_learner(self.cfg, self.dataset_handler, self.logger)
        self.trainer.print_model()

    def feTrIL_train_main(self):
        ...
        for task, original_imgs_train_dataset in enumerate(self.dataset_handler.original_imgs_train_datasets,
            ...
            if self.cfg.model.use_svm and not (self.cfg.use_base_half and task == int(self.dataset_handler.all_tasks / 2) or \
                    (not self.cfg.use_base_half and task == 1)):
                val_acc = self.trainer.validate_with_svm(task)
                val_acc_FC_str = f'task: {task} classififer:{"SVM"} val_acc: {val_acc}, avg: {val_acc.mean()} '
    ...
    def feTrIL_train_main_for_local_dataset(self):
        gpus = torch.cuda.device_count()
        self.logger.info(f"use {gpus} gpus")
        random.seed(self.cfg.seed)
        np.random.seed(self.cfg.seed)
