...
from lib.continual.NAPAVQ_learner import NAPAVQ_learner
from lib.dataset import TransformedDataset, AVAILABLE_TRANSFORMS


class NAPAVQ_handler:
    """Approach NAPA-VQ"""
    ...
    def handler_init(self):
        ...
        self.trainer = NAPAVQ_learner(self.cfg, self.dataset_handler, self.logger)
        self.trainer.print_model()

    def napavq_train_main(self):
        ...
        for task, original_imgs_train_dataset in enumerate(self.dataset_handler.original_imgs_train_datasets,
            ...
            if self.cfg.use_base_half and task == int(self.dataset_handler.all_tasks / 2) or \
                ...
                if self.cfg.train_first_task:
                    ...
                    self.trainer.load_napavq(self.cfg.task1_MODEL, self.cfg.task1_navq, train_dataset)
            else:
            ...
            val_acc = self.trainer.validate_with_navq(task)
            taskIL_FC_val_acc = self.trainer.validate_with_navq_taskIL(task)
            test_acc = None
            if self.dataset_handler.val_datasets:
                test_acc = self.trainer.validate_with_navq(task=task, is_test=True)
                taskIL_FC_test_acc = self.trainer.validate_with_navq_taskIL(task, is_test=True)
