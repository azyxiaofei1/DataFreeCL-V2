...
from lib.continual.FeCAM_learner import FeCAM_learner
...
class FeCAM_handler:
    ...
    def handler_init(self):
        ...
        self.trainer = FeCAM_learner(self.cfg, self.dataset_handler, self.logger)
        self.trainer.print_model()

    def fecam_train_main(self):
        ...
        for task, original_imgs_train_dataset in enumerate(self.dataset_handler.original_imgs_train_datasets,
            ...
            if (self.cfg.use_base_half and task == int(self.dataset_handler.all_tasks / 2) or \
                ...
                val_acc = self.trainer.validate_with_maha(task, active_classes_num)
                taskIL_FC_val_acc = self.trainer.validate_with_maha_taskIL(task, active_classes_num)
                test_acc_FC_str = f'task: {task} classififer:{"MAHA"} val_acc: {val_acc}, avg: {val_acc.mean()} '
                taskIL_test_acc_FC_str = f'task: {task} classififer:{"MAHA"} validate taskIL, val_acc: {taskIL_FC_val_acc}, ' \
                                         f'avg: {taskIL_FC_val_acc.mean()} '
                self.logger.info(test_acc_FC_str)
                self.logger.info(taskIL_test_acc_FC_str)
                pass

    def fecam_train_main_for_local_dataset(self):
        gpus = torch.cuda.device_count()
        self.logger.info(f"use {gpus} gpus")
        random.seed(self.cfg.seed)
        np.random.seed(self.cfg.seed)
        ...
        for task, original_imgs_train_dataset in enumerate(self.dataset_handler.original_imgs_train_datasets,
            ...
            if self.cfg.use_base_half and task == int(self.dataset_handler.all_tasks / 2) or \
                ...
                self.trainer.learn_new_task_for_local_dataset(train_dataset, active_classes_num, task,
                                                              train_dataset_transform=self.dataset_handler.val_test_dataset_transform)
            self.logger.info(f'#############MCFM train task {task} End.##############')
            self.logger.info(f'#############Example handler task {task} start.##############')
