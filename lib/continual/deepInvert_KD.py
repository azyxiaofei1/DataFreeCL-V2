...
class DeepInvert_KD(DeepInversionGenBN):

    def __init__(self, cfg, dataset_handler, logger):
        super(DeepInvert_KD, self).__init__(cfg, dataset_handler, logger)
    ...
    def train_main(self, train_dataset, active_classes_num):
        self.pre_steps(active_classes_num)
        ...
        self.deepInvert_KD(train_dataset, active_classes_num)

    def deepInvert_KD(self, train_dataset, active_classes_num):
        self.train_student(train_dataset, active_classes_num)
    ...
    def train_student(self, train_dataset, active_classes_num):
        # trains
        ...
        for epoch in range(1, self.cfg.student.TRAIN.MAX_EPOCH + 1):
            ...
            for x, y in train_loader:
                ...
                # ---> Train MAIN MODEL
                # data replay
                x_replay, y_replay, pre_logits_replay = self.sample(self.previous_teacher, len(x), active_classes_num)

                logits, features = self.student(x_replay)
                if self.cfg.student.use_cls:
                    loss_cls = criterion(logits, y_replay)
                else:
                    loss_cls = 0.
                if self.cfg.student.use_kd:
    ...
    def construct_pseudo_datasets(self, pseudo_dataset_root, active_classes_num):
        ...
        while min_class_sample < 100:
            x_replay, y_replay, pre_logits_replay = self.sample(self.previous_teacher, 128, active_classes_num)
            for index in range(len(y_replay)):
                ...
                if len(self.pseudo_datasets[label_item]) > 200:
                    self.pseudo_datasets[label_item] = self.pseudo_datasets[label_item][0:200]
        ...
        for label in range(active_classes_num):
            ...
            for img in class_dataset:
                inv_transformed_img = self.inv_normalize_transform(img)
                ndarr = inv_transformed_img.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to("cpu",
                                                                                                  torch.uint8).numpy()
                im = Image.fromarray(ndarr)
                im.save(os.path.join(fp, str(index) + ".png"))
                index += 1
