        ...
        for epoch in range(1, self.cfg.model.TRAIN.MAX_EPOCH + 1):
            ...
            for x, y in train_loader:
                ...
                if "ReKD" == self.cfg.model.KD_type:
                    ...
                    #                             sample_num_grade_classes,
                    #                             temp=self.cfg.model.T,
                    #                             dw_kd=None) * self.cfg.model.kd_lambda  # todo Done
                    ...
                    loss_kd = 0.
                if self.cfg.model.featureKD.use_featureKD:

                    if self.cfg.model.featureKD.use_ABD_KD:
                        self.FCTM.zero_grad()
                        ...
                        # all_outputs_for_dis = all_outputs[:, 0:dpt_classes_num]
                        # FCTM_out_for_dis = FCTM_outputs[:, 0:dpt_classes_num]
                        # loss_kd += self.cfg.model.fkd_lambda * compute_distill_loss(output_for_distill=all_outputs_for_dis,
                        #                                                             previous_task_model_output=FCTM_out_for_dis,
                        #                                                             temp=2.)

                        all_outputs_for_dis = all_outputs[:, 0:active_classes_num]
                        FCTM_out_for_dis = FCTM_outputs[:, 0:active_classes_num]
                        loss_kd += self.cfg.model.fkd_lambda * compute_distill_loss(
                            output_for_distill=all_outputs_for_dis,
                            previous_task_model_output=FCTM_out_for_dis,
                            temp=2.)
