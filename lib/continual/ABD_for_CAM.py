...
from lib.model import cam_resnet18, CrossEntropy, compute_distill_loss
from lib.utils import get_optimizer, get_scheduler, AverageMeter


class DeepInversionGenBN_for_CAM:
    ...
    def construct_model(self):
        model = cam_resnet18(class_num=self.dataset_handler.all_classes)
        return model
        pass
    ...
    def train_init(self):
        ...
        self.kd_criterion = torch.nn.MSELoss(reduction="none")
...
class ABD_for_CAM(DeepInversionGenBN_for_CAM):

    def __init__(self, cfg, dataset_handler, logger):
        super(ABD_for_CAM, self).__init__(cfg, dataset_handler, logger)
        self.criterion_fn = torch.nn.CrossEntropyLoss(reduction="none")
