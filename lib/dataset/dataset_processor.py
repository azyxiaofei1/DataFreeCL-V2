...
class SubDataset(Dataset):
    ...
    def __init__(self, original_dataset, sub_labels, target_transform=None):
        ...
        for index in range(len(self.dataset)):
            if hasattr(original_dataset, "targets"):#hasattr()用于检查一个对象是否具有某个指定的属性或方法
                if self.dataset.target_transform is None:
                    label = self.dataset.targets[index]#label=当前样本的标签比如 CIFAR100 的 0~99，label 就直接等于原始类别 id（0~99）。
                else:
    ...
    def __getitem__(self, index):
        sample = self.dataset[self.sub_indeces[index]]  #sample = [image_tensor, label]  # 或 (image_tensor, label)（看你的实现）
...
class TransformedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            input = self.transform(input)#转换函数执行图像预处理（如转为 Tensor、标准化）和/或数据增强（如随机裁剪、翻转）。
        if self.target_transform:
            target = self.target_transform(target)#如果已定义，说明有一个标签转换函数

        return input, target


class TransformedDataset_augment(Dataset):
    '''Modify existing dataset with transform; for creating multiple MNIST-permutations w/o loading data every time.'''

    def __init__(self, original_dataset, transform=None, target_transform=None, augment_transform=None):
        super().__init__()
        self.dataset = original_dataset
        self.transform = transform
        self.target_transform = target_transform
        self.augment_transform = augment_transform
        assert self.augment_transform is not None

    def __len__(self):
        return len(self.dataset)

    def __getitem__(self, index):
        (original_input, target) = self.dataset[index]
        if self.transform:
            input = self.transform(original_input)
        else:
            input = original_input
        if self.target_transform:
            target = self.target_transform(target)
        aug_input = self.augment_transform(original_input)
        return input, target, aug_input
