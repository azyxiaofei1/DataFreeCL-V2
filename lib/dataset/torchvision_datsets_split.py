...
class Torchvision_Datasets_Split:
    ...
    def __init__(self, cfg, split_selected_data=None):
        ...
        self.dataset_root = cfg.DATASET.data_root  # 数据路径
        self.all_classes = cfg.DATASET.all_classes  # 总类数（100）
        self.all_tasks = cfg.DATASET.all_tasks  # 总 task 数（比如 5）
        self.input_size = cfg.INPUT_SIZE  # (32,32) 或 (224,224)
        self.color_space = cfg.COLOR_SPACE  # 'RGB'
        self._split_selected_data = split_selected_data  # 手工指定的任务划分（可选）
        self.seed = cfg.DATASET.split_seed  # 划分种子
        self._val_length = cfg.DATASET.val_length  # 验证集长度
        self._use_svhn_extra = cfg.DATASET.use_svhn_extra  # SVHN 是否用 extra

        self.classes_per_task = None# 一个 task 中包含的类别数
        self.original_imgs_train_datasets = None# 原始训练集
        self.val_datasets = None# 验证集
        self.test_datasets = None# 测试集
        self.target_transform = None# 目标变换
        self.class_per_task_list = None# 一个 task 中包含的类别数列表
        ...
        if self.all_classes:
            ...
            for key, value in self._split_selected_data.items():
                ...
                if pre_classes is None:
                    ...
                    assert pre_classes == len(value), "存在类别数不相同的任务."
        self.classes_per_task = int(self.all_classes / self.all_tasks)# 一个 task 中包含的类别数



    def get_dataset(self):
        self.is_legal_initiate()
        self.class_per_task_list = []

        #1、决定每个 task 里有哪些类别
        if self._split_selected_data:
            self.class_per_task_list, ordered_class_list = self.get_determined_class_list()
        else:
            self.class_per_task_list, ordered_class_list = self.get_random_class_list()
        print(f"ordered_class_list: {ordered_class_list}")

        '''original class index mapping to class_index in current exp.'''

        #2、决定要不要重排 label（original_id → exp_id）
        original_classIndex_2_exp_classIndex = {}#作用：把「原始类别 id」映射到「本次实验重新排序后的 id」。
        """
            2: 0,   # 原始类2 → 实验里的第0类
            4: 1,   # 原始类4 → 实验里的第1类
            0: 2,   # 原始类0 → 实验里的第2类
        ...
        for exp_class_index in range(len(ordered_class_list)):
            if self.seed == 0 and self._split_selected_data is None:
                ...
                """
                ordered_class_list = [0,1,2,...,99]
                # 于是 assert ordered_class_list[i] == i
                # 不会往 original_classIndex_2_exp_classIndex 里填任何东西
        ...
        if self.seed == 0 and self._split_selected_data is None:
            ...
            """
            y：来自原始 dataset 的标签（original_id）
            p：那张“原始 id → 实验 id”的映射表 original_classIndex_2_exp_classIndex        
            p[y]：查表，把原始标签 y 映射为实验标签 exp_id
            int(...)：保证它是整数
            
            给一个原始标签y（y为原始标签，比如4）通过字典p（p为原始标签→实验标签的映射表）查表，{4:1} 得到实验标签exp_id=1，
            """
            print(f"self.target_transform:{self.target_transform}")

        #3、真正把原始数据读进来，并按任务「切开」
        self.original_imgs_train_datasets, self.val_datasets, self.test_datasets = \
            self.get_multitask_experiment_splited_dataset()


    # 处理“我手动指定好每个 task 里面有哪些类”
    def get_determined_class_list(self):#处理“我手动指定好每个 task 里面有哪些类”
    ...
    # 处理“我随机分配每个 task 里面有哪些类，但保证每个类都被分配到至少一个 task”
    def get_random_class_list(self):
        class_per_task_list = []
        order_list = np.array([class_index for class_index in range(self.all_classes)]) # 对于 CIFAR100：order_list = [0,1,2,...,99]
        """
        ...
        """
        if self.seed != 0:#若改变了种子，则重新打乱顺序，order_list = [52,1,30,72,4,5]
            print("Use random selected_class_list.")
            np.random.seed(self.seed)
            np.random.shuffle(order_list)#打乱顺序，order_list = [1, 4, 52, 30, 72]
        else:
            print("Use naturally ordered selected_class_list.")
        order_list = list(order_list)
        for task in range(self.all_tasks):#all_classes = 100, all_tasks = 5
            class_per_task_list.append(
                order_list[self.classes_per_task * task: self.classes_per_task * (task + 1)])# 对于 CIFAR100：class_per_task_list = [[0,1,2,...,19],[20,21,...,39],...,[80,...,99]]
        """
           task=0：order_list[0:20] → [0, 1, ..., 19]
           task=1：order_list[20:40] → [20, 21, ..., 39]
           task=2：order_list[40:60] → [40, ..., 59]
           task=3：order_list[60:80] → [60, ..., 79]
           task=4：order_list[80:100]→ [80, ..., 99]
           """
    ...
    def get_multitask_experiment_splited_dataset(self):
        ...
        if 'mnist' in dataset_name:
            ...
            val_datasets = None
            if self._val_length <= 0:
                for labels in self.class_per_task_list:
            ...
            if self._val_length <= 0:
                for labels in self.class_per_task_list:
                    ...
                    test_datasets.append(TransformedDataset(SubDataset(CIFAR10_test, labels),
                                                            target_transform=self.target_transform))
                    """
                    对某个样本 train_datasets[task_id][i] 的调用链是：
                    TransformedDataset.__getitem__
                    → 调 SubDataset.__getitem__      一次性筛好子集、以后高效重用,如只使用0~19类了 SubDataset，就不用每次训练都在整份数据集上遍历去找哪些样本属于当前任务的类。
                    → 再调 CIFAR100_train.__getitem__
            ...
            val_datasets = None
            """
            (这时按task切分)从 CIFAR100_train 全集里，筛掉所有 label 不在 labels 里的样本，只保留属于这些类的数据，形成一个子数据集。
            比如第一轮：
            labels = [0,1,...,19]         
            SubDataset(CIFAR100_train, labels) → “只包含类 0~19 的 CIFAR100 训练子集”。
            """
            if self._val_length <= 0:
                for labels in self.class_per_task_list:
    ...
    def write_split_selected_data(self, file_name):
        ...
        fw = open(file_name, 'w')
        self._split_selected_data = {}
        for task_id in range(len(self.class_per_task_list)):
            self._split_selected_data[task_id] = self.class_per_task_list[task_id]
    ...
    def get_selected_data(self):
        return self.class_per_task_list
...
def load_dataset(dataset_name, mode='train', download=True, dir='./datasets', image_transform=None,
    ...
    """
    AVAILABLE_DATASETS 是一个字典,所以 dataset_class 其实就是一个类，比如 torchvision.datasets.CIFAR100。
    AVAILABLE_DATASETS = {
